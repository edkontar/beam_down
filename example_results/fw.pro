...
LOADCT, 3  ;rainbow +white

;myMPEG = OBJ_NEW('IDLgrMPEG',  FILENAME='FW.mpeg') 
oVid = IDLffVideoWrite('FW.mp4')
width = 560
height=560
fps = 20
...
ee=vx*vx*9.1d-28*(2.2d10)^2/2./1.6e-9
ee=ee(0:Nv1-1)

window,2, xsize=400, ysize=600
!P.Multi=[0,1,3]

plot, ee,total(fxv,2),/xlog,/ylog,xrange=[1e-1,2e2]
oplot, ee,total(fxv0,2),line=1

dens_fxv=fltarr(Nv1,Nx)
for j=0, Nx-1 do dens_fxv(*,j)=dens(j)*fxv(*,j) 
...
fe=fe+total(dens_fxv,2)

plot, ee, fe,/xlog,/ylog

plot,ee,-deriv(alog(ee),alog(fe)),yrange=[-1,5],/xlog

!P.Multi=0

print,'Accelerated/injected ratio =', total(fxv(15:Nv1-1,*))/total(fxv0(15:Nv1-1,*))

;print,'Real/TT =', total(fxv(15:Nv1-1,*))/total(fxv0(15:Nv1-1,*))


IMAGE_CONT_MY, alog(Fxv+1e-10), /interp
grabed_image2= TVRD()

IMAGE_CONT_MY, Wxv(0:Nv1,0:Nx-3), /interp
grabed_image3 = TVRD()
window,0, xsize=560, ysize=560
TV, grabed_image2,0
TV, grabed_image3,1
times = string(format='(F4.2, A)', float(i)/100,' sec')
